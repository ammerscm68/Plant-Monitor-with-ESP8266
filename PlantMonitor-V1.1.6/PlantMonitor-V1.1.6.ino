...
void loop()
{
  ...
  // ###########################################################################
  // täglicher Neustart (04:00 Uhr)
  if  (String(h)+":"+String(m)+":"+String(s) == "4:0:0") {TimeRestart = true;}; 
  // ###########################################################################
  LoadValue += 1;
  if (LoadValue == 1 || LoadValue == soilMessureTrigger) // Messurement Interval
  {
  StartMessureTimer(); // Kunktion für die ersten Messungen nach dem Sensorstart
