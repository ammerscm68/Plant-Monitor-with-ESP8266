...
void setup()
{
  ...
  Serial.println("DNS-Responder und Web-Server starten ...");
  // Start DNS Client  - call with "HN.local" (Example: plant.local)  
  if (!MDNS.begin(HN)) {
  Serial.println("Error setting up MDNS responder!");
  while (1) {MyWaitLoop(1000);}}
  Serial.println("mDNS Responder gestartet - URL: http://"+HN+".local");
...
void loop()
{
  ...
  GetSoilMoistValue(5); // Messwert des Sensors auslesen
