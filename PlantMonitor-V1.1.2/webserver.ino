...
if (SoilCalibrate) {SoilCalibrate = false; GetSoilMoistValue();} // Nach dem Kalibrieren aktuellen Messwert holen
String RefreshTime = "";
...
SoilCalibrate = true;
 yield();
 MyWaitLoop(100); // kurze Pause sonst Absturz bei Browser refresh
 GetSoilMoistValue(); // Messwert Auslesen
 server.send(200, "text/html", 
  "<!DOCTYPE html>\
  <html>\
  <head>\
...
server.send(200, "text/html", 
  ...
  <h1>"+SoilSensorname+"</h1>\
  <a><br /></a>\
  <h2>Der Wert "+soilmoisthighlevel+" wurde für den 100% trocken Sensor übernommen und gespeichert.</h2>\
  <a href =\"/\"><button class=\"button\" style=\"margin-left: 10px;height: 28px; width: 180px;color: black; font-size: 14px\">zurück zur Startseite</button></a>\
  </body>\
  </html>\
  "); 
