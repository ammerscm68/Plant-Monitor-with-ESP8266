...
int soilMessureTrigger = 10; // Messintervall
...
void setup()
{
  ...
  Serial.println("Start Plant-Monitor ...");
  Serial.println("");
  Serial.println("");
  ...
  // Start DNS Server  - call with "SoilSensorname.local" (Example: Plant.local)
  Serial.println("DNS- und Web-Server starten ...");
  if (!MDNS.begin(cleanText(SoilSensorname))) {
  Serial.println("Error setting up MDNS responder!");
  while (1) {MyWaitLoop(1000);}}
  Serial.println("mDNS Responder gestartet - URL: "+cleanText(SoilSensorname)+".local");
  ...
  // Messurement-Trigger
  soilMessureTrigger = measurementinterval1 * 60; // Messintervall von Minuten in Sekunden umrechnen
...
void loop()
{
  ...
  else {BlinkLED(ROT);}}}} else {MQTTConnectFail = false;}} //  wenn kein MQTT
  ...
  if (!SoilCalibrate) {  
  GetSoilMoistValue(10); // Messwert des Sensors auslesen
  if (IsMQTT) {
